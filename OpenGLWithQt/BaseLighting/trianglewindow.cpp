...
void TriangleWindow::createShaderProgram()
{
    if ( !m_pgm.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/vshader.glsl")) {
        qDebug() << "Error in vertex shader:" << m_pgm.log();
        exit(1);
    }
    if ( !m_pgm.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/fshader.glsl")) {
...
void TriangleWindow::createGeometry()
{
